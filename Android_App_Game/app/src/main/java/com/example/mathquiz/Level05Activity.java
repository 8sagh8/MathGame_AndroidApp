...
public class Level05Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String text_name = intent.getStringExtra(Level04Activity.PLAYER_NAME);
        Integer text_score = getIntent().getExtras().getInt(Level04Activity.PLAYER_SCORE);

        // Set Player's Name and Score at top of screen
        TextView nameView = findViewById(R.id.details_taskBar_05);
        ...
        timer_Obj_02.schedule(new TimerTask() {
            @Override
            public void run() {
                final TextView _timing_05 = findViewById(R.id.timing_05);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (count_seconds < 11){
                            _timing_05.setText(String.valueOf(count_seconds));
                            ++count_seconds;
                        }
                    }
                });
        ...
        submit_Button =findViewById(R.id.submit_05);
        submit_Button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Integer int_text_score = getIntent().getExtras().getInt(Level04Activity.PLAYER_SCORE);
                ...
                EditText _answer = findViewById(R.id.answer_05);
    ...
    public void level_result(Integer int_text_score){
        Intent intent = getIntent();
        String text_name = intent.getStringExtra(Level04Activity.PLAYER_NAME);
        String text_age = intent.getStringExtra(Level04Activity.PLAYER_AGE);
