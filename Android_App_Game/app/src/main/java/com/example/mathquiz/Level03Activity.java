...
public class Level03Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String text_name = intent.getStringExtra(Level02Activity.PLAYER_NAME);
        Integer text_score = getIntent().getExtras().getInt(Level02Activity.PLAYER_SCORE);

        // Set Player's Name and Score at top of screen
        TextView nameView = findViewById(R.id.details_taskBar_03);
        ...
        timer_Obj_02.schedule(new TimerTask() {
            @Override
            public void run() {
                final TextView _timing_03 = findViewById(R.id.timing_03);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (count_seconds < 11){
                            _timing_03.setText(String.valueOf(count_seconds));
                            ++count_seconds;
                        }
                    }
                });
        ...
        submit_Button =findViewById(R.id.submit_03);
        submit_Button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Integer int_text_score = getIntent().getExtras().getInt(Level02Activity.PLAYER_SCORE);
                ...
                EditText _answer = findViewById(R.id.answer_03);
    ...
    public void level_04(Integer int_text_score){
        Intent intent = getIntent();
        String text_name = intent.getStringExtra(Level02Activity.PLAYER_NAME);
        String text_age = intent.getStringExtra(Level02Activity.PLAYER_AGE);

        // if all above conditions are okay.. then we will move on next page
        intent = new Intent(this, Level04Activity.class);
